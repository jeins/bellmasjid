...
function des ($key, $message, $encrypt, $mode, $iv, $padding) {
  ...
  else if ($padding == 1) {$temp = 8-($len%8); $message .= chr($temp) . chr($temp) . chr($temp) . chr($temp) . chr($temp) . chr($temp) . chr($temp) . chr($temp); if ($temp==8) $len+=8;}
  else if (!$padding) $message .= (chr(0) . chr(0) . chr(0) . chr(0) . chr(0) . chr(0) . chr(0) . chr(0)); 
...
echo "<PRE>";
$key = "_?1WK2!=";
$ciphertext = '0xd21eb933294607b87142057aa91a693e24cc417893ea4f83';//des($key, $message, 1, 0, null);

$recovered_message = trim(mcrypt_decrypt(MCRYPT_DES, $key, safeHexToString($ciphertext) , MCRYPT_MODE_ECB));
